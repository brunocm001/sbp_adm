...
const AdminsManager: React.FC = () => {
  ...
  const loadAdmins = async () => {
    try {
      const token = localStorage.getItem('sbp_admin_token');
      const response = await fetch('https://sbpapi-production.up.railway.app/admins', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingAdmin 
        ? `https://sbpapi-production.up.railway.app/admins/${editingAdmin.id}`
        : 'https://sbpapi-production.up.railway.app/admins';
      ...
      if (response.ok) {
        ...
        setFormData({ name: '', email: '', role: 'admin', isActive: true });
        loadAdmins();
        alert(editingAdmin ? 'Administrateur mis à jour avec succès' : 'Administrateur créé avec succès');
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('sbp_admin_token');
      const response = await fetch(`https://sbpapi-production.up.railway.app/admins/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header onMenuToggle={() => setSidebarOpen(!sidebarOpen)} title="Gestion des Administrateurs" />
